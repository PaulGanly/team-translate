...
public interface TranslationApi {
	...
	@RequestMapping(value = "/translation/{id}", produces = { "application/json" }, method = RequestMethod.GET)
	ResponseEntity<Translation> getTranslationById(@PathVariable("id") Long id);
	...
	ResponseEntity<Void> inputTranslationWithForm(@ModelAttribute(value = "english") String english,
			@ModelAttribute(value = "irish") String irish, @ModelAttribute(value = "context") String context);

	@RequestMapping(value = "/translation/uploadFile", produces = { "application/json" }, consumes = {
			"multipart/form-data" }, method = RequestMethod.POST)
	ResponseEntity<UploadResponse> uploadFile(@RequestPart(value = "language", required = true) String language,
